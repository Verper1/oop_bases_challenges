...
def test__make_username_capitalized__username_capitalized(test_username: str) -> None:
    user = User(user_id=1, username=test_username, name="Валера")  # Arrange

    username_capitalized = user.make_username_capitalized()  # Act
...
def test__generate_short_user_description__short_user_description_was_given(
        ...
        test_user_id: int,
        test_name: str) -> None:
    user = User(user_id=test_user_id, username=test_username, name=test_name)  # Arrange

    short_user_description = user.generate_short_user_description()  # Act
